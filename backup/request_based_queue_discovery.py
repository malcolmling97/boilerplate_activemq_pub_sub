    ...
    if listener.queues:
        ...
        for queue_name, stats in sorted(listener.queues.items()):
            ...
            if not queue_name.startswith('ActiveMQ.'):
                ...
                other_stats = {k: v for k, v in stats.items() if k not in printed_keys and k not in ['destinationName', 'brokerId', 'brokerName', 'memoryLimit']}
